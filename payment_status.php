	...
	if ($currency == 2) {
		...
		if (isset($_GET['success']) && isset($_GET['paymentId']) && isset($_GET['PayerID'])) {
				...
				if ($count1 > 0) {
					...
					if ($stmtU->execute()) {
						...
						if ($stmtAP->execute()) {
							...
							$sql2 = "SELECT DISTINCT ap.app_id, ap.id FROM master_payment mp join author_payment ap on ap.master_payment_id=mp.id WHERE ap.payment_done='1' and mp.order_id='$order_id'";
							...
							if (!empty($results2)) {
								...
								foreach ($results2 as $key => $value) {
									$sql5 = "SELECT ap.plan_id,pl.validity_in_days FROM author_payment ap join plans pl on pl.id=ap.plan_id WHERE ap.app_id='" . $value['app_id'] . "' AND ap.payment_done=1 AND ap.plan_id<>'' ORDER BY ap.id DESC LIMIT 1";
									...
									if (!empty($resultsInv)) {
										...
										*/
										$queryInv12 = 'SELECT a.id,mp.id as masterpaymentid,mp.promocodes_id,a.plan_id,SUM(ad.amount) AS breakup_some,a.total_amount AS plan_amount, a.platform,
										GROUP_CONCAT( 
										CASE 
										...
										if (!empty($resultsInv12)) {
											...
											<p style="margin:10px 0 0 20px;">"' . $resultsApp['summary'] . '"</p>
											<p style="margin:10px 0 0 20px;">"' . $my_app_type . '"</p>
											<p style="margin:10px 0 0 20px;">OS <b>' . $p_platform . '</b></p>
											<p style="margin:10px 0 0 20px;">Package <b>' . $resultsInv['plan_name'] . '</b></p>';
					...
					//$basicUrl = $this->url;
					$chtmlcontent = file_get_contents('edm/app_qa.php');
					...
					$mpdf->WriteHTML(file_get_contents("test.php"));
			...
			//
									...
									$sql = 'SELECT sum(part_amount) as total  FROM master_payment_part WHERE app_id="' . $maser_app_id . '" and payment_done=1';
	                             ...
	                             $results_pm=$ack_invoice->part_payment_details($maser_app_id);
									...
									if (!empty($results_cplan)) {
												...
												$queryInv123 = 'SELECT * FROM app_type WHERE id="' . $resultsApp['type_app'] . '"';
													...
													$order_items .= '<tr>
														...
														<p style="margin:10px 0 0 20px;">"' . $resultsApp['summary'] . '"</p>
														<p style="margin:10px 0 0 20px;">"' . $my_app_type . '"</p>
														<p style="margin:10px 0 0 20px;">OS <b>' . $results_platform['name'] . '</b></p>
														<p style="margin:10px 0 0 20px;">Package <b>' . $results_cplan['plan_name'] . '</b></p>';
														...
														<td valign="top" style="font-family:Arial, Helvetica, sans-serif; color:#323232  ; font-size:15px; padding:30px 0; border-right:1px solid #dbdbdb  ; border-bottom:1px solid #dbdbdb  ;">1</td>
														<td valign="top" style="font-family:Arial, Helvetica, sans-serif; font-size:15px; color:#323232  ; padding:30px 0; border-right:1px solid #dbdbdb  ; border-bottom:1px solid #dbdbdb  ;">' . $yyyyy . '</td>
											<td valign="top" style="font-family:Arial, Helvetica, sans-serif; font-size:15px; padding:30px 0; border-right:1px solid #333; border-bottom:1px solid #dbdbdb  ; color:#323232  ;"> ' . $results_curr['Name'] . ' ' . round($results_part_all['total'], 2) . '</td>
														</tr>';
									...
									$planimg="market_packages2.png";
									// invoice email
									$csubject = 'Instappy  Invoice for  Advance Payment order ' . $order_id;
									//$basicUrl = $this->url;
									$chtmlcontent = file_get_contents('edm/invoice.php');
									$clastname = $results['last_name'] != '' ? ' ' . $results['last_name'] : $results['last_name'];
									$cname = ucwords($results['first_name'] . $clastname);
									...
									$chtmlcontent = str_replace('{order_total}', ((round($results_part_all['total'], 2)+round($results_part_all['total']*$service_tax/100, 2))-$results6['discount']), $chtmlcontent);
									$chtmlcontent = str_replace('{currentdate}', date('d-m-Y', time()), $chtmlcontent);
								...
								$app_amount=0;$kk=0;$not_done=0;$done=0;$left_amt=0;
                        		...
                        		<div class="right_inner">
									<div class="payment_box">
										<div class="payment_left view-area">
											...
											<span>
											Thank you for the prompt payment on your Instappy account. This represents the <?php echo $done; ?> instalment of amount <?php  echo $currencyIcon.''.$_SESSION['part_total_amount'];?> on your plan, bringing your remaining balance to <?php echo $currencyIcon.''.
											($left_amt+$left_amt*$service_tax/100); ?> out of <?php echo $currencyIcon.''.($app_amount+$app_amount*$service_tax/100); ?>.  We will contact you again on <?php echo $next_part_payment; ?> when your next payment is due.</span>
											<?php }
											else{
											?>
		...
		if (($orderstatus != '') && ($tracking_id != '') && ($order_id != '')) {
			if ($orderstatus === "Success") {
				...
				$basicUrl = $db->siteurl();
				 ...
				 $sql6 = "SELECT * FROM master_payment WHERE payment_done!='1' AND order_id='$order_id'";
				$stmt6 = $mysqli->prepare($sql6);
				$stmt6->execute();
				$results6 = $stmt6->fetch(PDO::FETCH_ASSOC);
				$promocode = $results6['promocodes_id'];
						...
						if ($stmtAP->execute()) {
							...
							if (!empty($results2)) {
								foreach ($results2 as $key => $value) {
									...
									if($is_partpayment==1){
										...
										if($results_planc['is_custom']==1){
											$sql_cplan= "SELECT id,validity_in_days FROM custom_plans WHERE  id='".$planID."' LIMIT 1";
											 ...
											 $sql_cplan= "SELECT id,validity_in_days FROM plans WHERE  id='".$planID."' LIMIT 1";
										}
										...
										//plane ckeck end
										 ...
										 $queryAP = 'UPDATE master_payment_part set  payment_done=1 ,updated_at=now() ,transaction_id="' . $tracking_id . '",transaction_date=NOW() WHERE master_payment_id='.$results6['id'].' AND app_id='.$results_part['app_id'].' AND payment_done=0 LIMIT 1';
										$stmtAP = $mysqli->prepare($queryAP);	
										$stmtAP->execute();
										$maser_part_id=$results_part['id'];
					 					$maser_app_id=$results_part['app_id'];
									...
									$queryInv = 'SELECT * FROM plans WHERE id="' . $planID . '"';
									$stmtInv = $mysqli->prepare($queryInv);
									$stmtInv->execute();
									$resultsInv = $stmtInv->fetch(PDO::FETCH_ASSOC);
									if (!empty($resultsInv)) {
								...
								<div class="payment_box">
									<div class="payment_left view-area">
										<div class="payment_thankyou">
											...
											<p>Your awesome app is up and ready.</p>
											<?php if($next_part_payment){?>
											<span>Thank you for the prompt payment on your Instappy account. This represents the <?php echo $done; ?> instalment of amount <?php  echo $currencyIcon.''.$_SESSION['part_total_amount'];?> on your plan, bringing your remaining balance to <?php echo $currencyIcon.''.
											($left_amt+$left_amt*$service_tax/100); ?> out of <?php echo $currencyIcon.''.($app_amount+$app_amount*$service_tax/100); ?>.  We will contact you again on <?php echo $next_part_payment; ?> when your next payment is due.</span>
											<?php }
											else{
											?>
