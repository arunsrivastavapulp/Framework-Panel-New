...
var nicLinkButton = nicEditorAdvancedButton.extend({	
	...
	submit : function(e) {
		...
		var urlreg = new RegExp("/^(http(s)?:\/\/)?(www\.)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/");
		if(url == "http://" || url == "") {
		...
		if(!this.ln) {
			...
			this.ln = this.findElm('A','temp href',tmp);
			if($('#present').find('a').length != 0){
				if($('#present').find('a').attr('href')) {
					$('#present').find('a').attr('target','_blank');
					$('#present').find('a').each(function(){
						var editorLink = $(this).attr('href');
                        $(this).removeAttr('target');
                        $(this).removeAttr('href');
                        $(this).attr('data-link',editorLink);
						$(this).attr('href',editorLink);
