  ...
  return function(zStack, options, doc, queue, _html2canvas) {
    ...
    if (queueLen === 1) {
      if (typeof options.elements[0] === "object" && options.elements[0].nodeName !== "BODY") {
        ...
        ctx.drawImage(canvas, Math.max(1, Math.floor(bounds.left)), Math.max(1, Math.floor(bounds.top)), Math.max(1, Math.floor(bounds.width)), Math.max(1, Math.floor(bounds.height)), 0, 0, Math.max(1, Math.floor(bounds.width)), Math.max(1, Math.floor(bounds.height)));
        canvas = null;
