...
function updateSlideNames() {
	...
	for (var i = 0; i < pageDetails.length; i++)
	{
		...
		if (linkValue != 'Create Category' && isNaN(linkValue) == false)
		{
			...
			if(i > 0)
			{
				$(".menu_body_links").eq(i).find(".deletScreen").show();
			}
		...
		else
		{
			...
			if (selectcatText != '')
			{
				$(".navCategoryList").eq(i).find('option[value="Create Category"]').prop('selected', true);
				...
				$("nav > ul li").eq(i).find('p').each(function (index) {
					...
					if (pval != 'Create Category' || pval != 'Choose Screen') {
						// $('.viewdelete').show(); 
					}
					if (index != 0) {
						//var dropHtml = '';
						...
						if ($(".menu_body_links").eq(i).find(".navSubCategoryList").length == 0) {
							$(dropHtml).insertAfter($(".menu_body_links").eq(i).find(".nav-cat_name"));
						} else {
		...
		if (typeof className == 'undefined' || className == 'icon-report' || className.indexOf("undefined") > 0)
		{
	...
	$('.navSubCategoryList').each(function(){
		if($(this).find('.extraDrop').length == 0)
		{
			var newhtml = $(this).clone();
			$(newhtml).insertAfter($(this).nextAll('.viewScreen')).addClass('extraDrop').prop('disabled', false);
		}
	});
...
function firstUpdateSlideNames()
{
	...
	$("nav > ul li").each(function (index) {
		...
		if (isNaN(linkValue) == false || linkValue == 'Create Category') {
			checkVal = index + 1;
...
//		}
	...
	$(".confirm_delete_screen_button").on("click", function () {
		...
		setTimeout(function () {
			$('nav > ul').find("[data-link='" + $("#presentRow").parents(".menu_body_links").find(".navCategoryList").val() + "']").remove();
			var temp = $("#presentRow").parents(".menu_body_links").find(".navCategoryList").val();
			deleteHtml(temp);
	...
	$(".addToSlide").on('change', function () {
		...
		if (currentClick.prop("checked") == true)
		{
			...
			currentClick.parents(".menu_body_links").find(".navSubCategoryList").prop('disabled', true);
			...
			if (currentNav == 'Create Category')
			{
				...
				if (categoryvalue != '')
				{
					...
					$("<li cat-index='" + categoryIndex + "' data-link='Create Category' ><a href='#'><span class='" + currentClick.parent().next().children('span').prop('class') + "'></span><span>" + categoryvaluelist + "</span><div class='clear'></div></a></li>").insertAfter("nav > ul li:nth-child("+parseInt(categoryIndex-1)+")");
				} 
				else
				{
		...
		else if ($(this).prop("checked") == false)
		{
			...
			if (selectedVal == 'Create Category' || selectedVal == 'Choose Screen')
			{
...
//	});
	...
	$(".navCategoryList").on('change', function () {
		var ind = $(this).index(".navCategoryList");
		...
		var subcat = $(this).parents(".menu_body_links").find(".navSubCategoryList").length;
		for (var i = 0; i < subcat; i++) {
			var valueOfcat = $(this).parents(".menu_body_links").find(".navSubCategoryList option:selected").eq(i).val();
		...
		if (catOption == 'Create Category') { 
			$(this).siblings('.viewScreen').hide();
			$(this).siblings('.deletScreen').hide();
			
			$(this).siblings('.nav-cat_name').css("display", "block");
			var dropHtml = $('.navSubCategoryList').eq(0).clone(true).attr("data-subcat", 0);
			$(this).siblings('.navSubCategoryList').show();
			...
			$(this).siblings('.viewScreen').show();
			$(this).siblings('.deletScreen').show();
			...
			$(this).siblings('.nav-cat_name').css("display", "none");
			$(this).siblings('.navSubCategoryList').remove();
...
$(document).on('change', '.navSubCategoryList', function () {
	...
	//var allsubcatOption = ''; 
	$(this).parents(".navText").find(".navSubCategoryList option:selected").each(function (index) {
...
$(document).on('click', '.deletScreen', function () {
	if($(this).parents('.menu_body_links').find('.addToSlide').prop('checked') == true)
	{
		alert('Please remove it from navigation to continue');
	}
	else
	{
		var myindex = $(this).attr("delete-ind");
		...
		$(this).prev(".navSubCategoryList").remove();
		$(this).nextAll('.viewScreen').remove();
		$(this).remove();
	}
