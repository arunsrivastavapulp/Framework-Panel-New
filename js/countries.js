...
function populateStates(countryElementId, stateElementId,stateValue) {
    ...
    for (var i = 0; i < state_arr.length; i++) {
		if(stateValue == state_arr[i]){
			stateElement.options[stateElement.length] = new Option(state_arr[i], state_arr[i],true,true);
		}
		else{
			stateElement.options[stateElement.length] = new Option(state_arr[i], state_arr[i],false,false);
		}
        
    }
}

function populateCountries(countryElementId, stateElementId,countryValue,stateValue) {
    ...
    for (var i = 0; i < country_arr.length; i++) {
        if(countryValue == country_arr[i]){
			...
			countryElement.options[countryElement.length] = new Option(country_arr[i], country_arr[i],false,false);
		}
    ...
    if (stateElementId) {
		if(stateValue !=''){
			populateStates(countryElementId, stateElementId,stateValue);
		}
