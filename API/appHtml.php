...
//	{
			...
			if($navigation_html != '')
						...
						if ($calRow == 0 && $screen_id !=0) 
									{
													$sqlUserInsert = "INSERT INTO `app_screenmapping_html` ( `app_id`, `screen_sequence`,`title`,`banner_html`, `navigation_html`,`content_html`, `autherId`, `linkto_screenid`,`layoutType`,`PageIndex`)
													VALUES ( '" . $app_id . "', '" . $screen_id . "', '" . addslashes($title) . "','" . addslashes($banner_html) . "','" . addslashes($navigation_html) . "','" . addslashes($htmlvalues) . "', '" . $author . "', '" . $LinkTo . "','" . $layoutType . "','".$originalIndex."')";
													 $statementUserInsert = $dbCon->prepare($sqlUserInsert);
													$statementUserInsert->execute();
														$sqlUserInsert_ach = "INSERT INTO `app_screenmapping_html_ach` ( `app_id`, `screen_sequence`,`title`,`banner_html`, `navigation_html`,`content_html`, `autherId`, `linkto_screenid`,`layoutType`,`PageIndex`)
													VALUES ( '" . $app_id . "', '" . $screen_id . "', '" . addslashes($title) . "','" . addslashes($banner_html) . "','" . addslashes($navigation_html) . "','" . addslashes($htmlvalues) . "', '" . $author . "', '" . $LinkTo . "','" . $layoutType . "','".$originalIndex."')";
													$statementUserInsert_ach = $dbCon->prepare($sqlUserInsert_ach);
													$statementUserInsert_ach->execute();
										 } 
									elseif($htmlvalues != '' || !empty ($htmlvalues)) 
											{
												$sqlUserInsert_ach = "update app_screenmapping_html_ach set title='" . addslashes($title) . "',`banner_html`='" . addslashes($banner_html) . "', `navigation_html`='" . addslashes($navigation_html) . "',`content_html`='" . addslashes($htmlvalues) . "',`linkto_screenid`='" . $LinkTo . "',`layoutType`='" . $layoutType . "',`PageIndex`='".$originalIndex."', `deleted`=0 where `app_id`='" . $app_id . "' and `screen_sequence`='" . $screen_id . "'";
												$statementUserInsert_ach = $dbCon->prepare($sqlUserInsert_ach);
