...
WHERE ad.screen_id='" . $screenId . "' AND ad.app_id='" . $app_id . "' AND DATE(start_datetime)>=CURDATE() order by ar.start_datetime ASC limit $offset,15";
            ...
            foreach ($result_eventAll as $resultAllEventSet) {
                ...
                if ($resultAllEventSet->event_type_id == 2 || $resultAllEventSet->event_type_id == 3) {
                    ...
                    if ($differTime == 0) {
                        ...
                        $componentResultSet[] = array("comp_id" => $compId, "comp_type" => $compType, "tag" => '', "linkto_screenid" => $resultAllEventSet->linkto_screenid, "pos" => "$pos", "comp_elements" => $arraycomp_elements);
                    }
                } else {
                    $eventhours = date('H', $date);
                    if ($eventhours == 00) {
...
WHERE
    app_id = '" . $app_id . "' AND screen_id = '" . $screenId . "'
...
ORDER BY component_no , list_no , componentarraylink_id";
            ...
            $componentQueryExecution = $dbCon->query($componentQuery);
...
ar.image1,ar.image2,ar.allday,ar.event_type_id,et.NAME AS event_type_name,ar.updatable
 FROM app_event_rel ad LEFT JOIN event_data ar ON ad.event_data_id=ar.id LEFT JOIN event_type et ON ar.event_type_id=et.id
WHERE ad.screen_id='" . $resultSet->linkto_screenid . "' AND ad.app_id='" . $app_id . "' AND DATE(start_datetime)>=CURDATE() order by ar.start_datetime ASC
LIMIT 3
";
                        ...
                        $result_event = $app_eventExecution->fetchAll(PDO::FETCH_OBJ);
                            ...
                            if ($eventSet->event_type_id == 2 || $eventSet->event_type_id == 3) {
                                $eventhours = date('H', $date);
                                if ($eventhours == 00) {
                                    $myTime = date('H:i', $date);
                                    // $myTime = date_format($date, 'H:i');
                            ...
                            $headingEvent[] = array("heading" => array("text" => $eventSet->title, "font_size" => $resultSet->font_size, "font_color" => $resultSet->font_color, "font_typeface" => $resultSet->font_typeface), "subheading" => array("text" => $myTimeFinal, "font_size" => $resultSet->font_size, "font_color" => $resultSet->font_color, "font_typeface" => $resultSet->font_typeface));
					...
					elseif ($resultSet->component_attr_name != NULL && $resultSet->list_type != 1) {
                        ...
                        if ($resultSet->component_type_name == 'Calendar View Card ') {
                            ...
                            if ($resultSet->component_attr_name == 'date') {
                                ...
                                for ($i = 0; $i < $myArrayCount; $i++) {
                                    if ($myArray[$i] == 'text') {
                                        $appdescription = "SELECT title,start_datetime FROM event_data ar LEFT JOIN event_type et ON ar.event_type_id=et.id WHERE title LIKE '%sunrise%' AND DATE(start_datetime)=CURDATE()LIMIT 1";
                                        $app_desExecution = $dbCon->query($appdescription);
                                        $resudes = $app_desExecution->fetchAll(PDO::FETCH_OBJ);
                                        $countDesNav = count($resudes);
                                        if ($countDesNav != 0) {
                                ...
                                for ($i = 0; $i < $myArrayCount; $i++) {
                                    if ($myArray[$i] == 'text') {
                                        $appdescription = "SELECT title,start_datetime FROM event_data ar LEFT JOIN event_type et ON ar.event_type_id=et.id WHERE title LIKE '%Sunset%' AND DATE(start_datetime)=CURDATE() LIMIT 1 ";
                                        $app_desExecution = $dbCon->query($appdescription);
                                        $resudes = $app_desExecution->fetchAll(PDO::FETCH_OBJ);
                                        $countDesNav = count($resudes);
                                        if ($countDesNav != 0) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                    ...
                    $componentResultSet[] = array("comp_id" => $compId, "comp_type" => $compType, "tag" => '', "linkto_screenid" => $linkTo, "pos" => $pos, "comp_elements" => $arraycomp_elements);
                    $arraycomp_elements = array();
