...
function index() {
 ...
 order by th.ordering ASC";
    ...
    foreach ($themeResult as $value) {
		...
		$uniqueArray = array("id" => $value['themeId'], "name" => $value['name'], "image" => $value['image'],"image_new" => $value['image_new'], "image2x" => $value['image2x'],"featured" => $value['is_featured'], "parent_cat" => array($parent_cat),"top_cat"=>array("id"=>$value['parent_id'],"name"=>$value['parent_category']));
