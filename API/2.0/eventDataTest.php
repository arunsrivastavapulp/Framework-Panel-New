...
WHERE DATE(start_datetime)='" . $years."-".$months."-".$_POST['day'] ."'   and ad.app_id='" . $app_id . "' order by ar.start_datetime ASC ";
...
WHERE (ar.start_datetime BETWEEN '" . $years . "-$months-01 00:00:00" . "' AND '" . $years . "-$months-31 23:59:59" . "') and ad.app_id='" . $app_id . "'   order by ar.start_datetime ASC ";
	$appEventmooon = "select  ed.image1,ed.start_datetime,et.id as event_type,et.is_visible from event_data ed   left join     event_type et    on    ed.event_type_id=et.id  where event_type_id=6 and start_datetime BETWEEN '" . $years . "-$months-01" . "' AND '" . $years . "-$months-31" . "' order by start_datetime ASC";
$app_Alleventmoon = $dbCon->query($appEventmooon); 
            $moonAll = $app_Alleventmoon->fetchAll(PDO::FETCH_OBJ);
	}
			...
			$resultSetcount= count($result_eventAll);	
if($resultSetcount == 0)
{
$Allldata=array("calendar_date" => "no data","moonData"=>$moonAll);
 $response = array("result" => 'success', 'msg'=>'');
 
        $Basearray = array("response" => $response,"data" => $Allldata);
        $basejson = json_encode($Basearray);
        echo $basejson;
		die;
}
...
{ 
            ...
            foreach ($result_eventAll as $resultAllEventSet) {
                if ($resultAllEventSet->image1 != '' || $resultAllEventSet->image1 != NULL) {
                    ...
                    if ($resultAllEventSet->event_type_id != 3) {
                        $date = strtotime($resultAllEventSet->start_datetime);
                        $eventhours = date('H', $date);
                        if ($eventhours == 00) {
                           $myTime = date('H:i', $date);
