...
/*
 * This class includes all the basic functions used in api
 * Added By Arun Srivastava on 3/12/15
 */
class mainFunctions
{	
    /*
     * check query run for api
	...
	/*
	 * convert array to json, outputs in all string formats (integer and float value is also handled)
	 * '/("\w+"):(\d+)/', '\\1:"\\2"' is replaced by '/\: *([0-9]+\.:?[0-9e+\-]*)/', ':"\\1"'
	 * Added By Arun Srivastava 10/8/15
	 */
	...
	/*
	 * function to add data in users, user_appdata tables
	 */
	...
	function loginCheck($appID, $email, $pwd)
	{
		...
		if($appID != '')
		{
			//$auth_query = "select * from user_appdata uad where uad.email_address = '" . $email . "' AND uad.password='" . md5($pwd). "' AND uad.app_id = '".$appID."'";
			$auth_query = "select * from user_appdata uad where uad.email_address = '" . $email . "' AND uad.app_id = '".$appID."'";
			$auth_queryExecution = $this->query_run($auth_query, 'select');  
			//if($auth_queryExecution['is_verified']>0)
