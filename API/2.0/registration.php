...
function getRegister() {
    ...
    if ($device_id != '' && $Framework_Version != '' && $plateform != '' && $app_idString != '') {
        ...
        $Basearray = array("response" => $response, "login_enable" => $enableLogin,"timeout"=>"15" ,"login_details" => array("login_method" => $loginMethod, "logo_url" => $logoUrl, "description" => $description), "register" => array("success" => '200', "app_type" => "" . $app_type . "", "device_id" => "" . $device_id . "", "EncryptoAuth" => "" . $encrpt_token . ""));
        $basejson = json_encode($Basearray);
        echo $basejson;
    } else {
        $response = array("result" => 'error', "msg" => 'parameter missing or wrong parameter');
