...
if ($s3->putObjectFile($fileTempName, "instappy", $fileName, S3::ACL_PUBLIC_READ)) {
  list($width, $height) = getimagesize($directory);
        echo json_encode(array('imageurl' => 'https://s3-us-west-2.amazonaws.com/instappy/'.$fileName ,'height' => $height, 'width' => $width));
        unlink($file);
		unlink($directory);
			if (strpos($a, 'panelimage') !== false) {
			unlink($oldUrl);
			}
			elseif(strpos($a, 'instappy') !== false)
			{
			$r= explode('instappy/',$oldUrl);
			$uri= $r[1];	
			deleteObject("instappy", $uri);
			}
